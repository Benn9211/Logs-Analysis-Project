...
query_1 = 
    ...
    "mangas.title, log.path order by views desc limit 3")

# Who are the most popular mangas authors of all time?
query_2_title = ("Who are the most popular mangas authors of all time?")
query_2 = (
    "select authors.name, count(*) as views from mangas inner "
    "join authors on mangas.author = authors.id inner join log "
    "on log.path like concat('%', articles.slug, '%') where "
    "log.status like '%100%' group "
...
query_3 = (
    ...
    "count(*) as requests from log where status like '%50%' group by day)"
...
if __name__ == '__main__':
    # store query results
    popular_mangas_results = get_query_results(query_1), query_1_title
